...
class BayesianCredibilityEngine:
    """
    Sophisticated Bayesian credibility scoring with:
    - Prior probabilities by domain category
    - Likelihood estimation from signals
    - Posterior updates with evidence
    ...
    def score_document(
        ...
        features = self._extract_features(url, title, abstract, full_text, metadata)
        ...
        # 5) Incorporate citation network influence
        network_adjustment = self._network_influence(url, citations_to, citations_from)
        posterior = 0.85 * posterior + 0.15 * network_adjustment
    ...
    def _extract_features(
        ...
        # URL features
        domain = self._extract_domain(url)
        features["domain_age_signal"] = self._estimate_domain_age(domain)
    ...
    def _estimate_domain_age(self, domain: str) -> float:
        """Estimate domain age credibility (old = more established)."""
        # Simple heuristic: known old domains
        established_domains = [
            "nature.com", "arxiv.org", "pubmed.ncbi.nlm.nih.gov",
            "bbc.com", "reuters.com", "github.com"
        ]
    ...
    def _domain_reputation_score(self, domain: str) -> float:
        ...
        return min(1.0, 0.7 + 0.1 * edu_org_gov)

    def _calculate_domain_entropy(self, domain: str) -> float:
        """Lower entropy = more memorable/established."""
        # Simple heuristic: longer domains = higher entropy
        entropy = min(1.0, len(domain) / 50)
        return 1.0 - entropy  # Invert (lower entropy = higher credibility)
    ...
    def _measure_specificity(self, text: str) -> float:
        ...
        # Check for numbers, dates, specific claims
    ...
    def _measure_neutrality(self, text: str) -> float:
        ...
        sensational = [
            "shocking", "amazing", "unbelievable", "viral",
    ...
    def _detect_methodology(self, text: str) -> float:
        ...
        match = len(re.findall(
            r'\b(method|approach|design|study|experiment|procedure)\b',
            text,
            re.I
        ))
    ...
    def _detect_results(self, text: str) -> float:
        ...
        match = len(re.findall(
            r'\b(result|finding|show|demonstrate|conclude|find)\b',
            text,
            re.I
        ))
        return min(1.0, match / 3)
    ...
    def _detect_limitations(self, text: str) -> float:
        ...
        match = len(re.findall(
            r'\b(limit|caveat|future work|further research|constraint)\b',
            text,
            re.I
        ))
    ...
    def _measure_reference_quality(self, text: str) -> float:
        ...
        refs = len(re.findall(r'\[\d+\]|et al\.|doi:', text))
    ...
    def _measure_coherence(self, text: str) -> float:
        ...
        transitions = len(re.findall(
            r'\b(however|therefore|consequently|moreover|furthermore|thus)\b',
            text,
            re.I
        ))
    ...
    def _calculate_likelihoods(self, features: Dict[str, float]) -> tuple:
        ...
        for signal, value in features.items():
            ...
            if signal in self.signal_likelihoods:
                ...
                p_high = value + 0.2  # High credibility correlates with signal
                p_low = (1 - value) * 0.8
    ...
    def _network_influence(
        ...
        You citing many sources = boost.
        """
        network_score = 0.5  # Neutral default

        if citations_to:
            network_score += 0.1 * min(1.0, len(citations_to) / 10)

        if citations_from:
            network_score += 0.2 * min(1.0, len(citations_from) / 5)

        self.citation_network[url] = {
            "cites": citations_to or [],
            "cited_by": citations_from or []
        }
    ...
    def _categorize_domain(self, domain: str) -> str:
        patterns = {
            "academic": ["arxiv", "pubmed", "nature", "scholar", ".edu", ".ac."],
            "news": ["bbc", "reuters", "nytimes", "guardian", "ft.com"],
            "code": ["github", "gitlab", "stackoverflow"],
            "forum": ["reddit", "ycombinator"],
            "blog": ["medium", "substack", "wordpress"],
        }
...
def register_tool(registry):
    engine = BayesianCredibilityEngine()

    registry.register("cred_bayesian", engine.score_document)

    logger.info("Registered Bayesian credibility engine (real-time learning)")

...
class FileManager:
    ...
    async def delete_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not path.exists():
                return {"success": False, "message": f"Error: File not found: {file_path}", "path": str(path)}
            
            if not path.is_file():
                return {"success": False, "message": f"Error: Not a file: {file_path}", "path": str(path)}
            ...
            return {"success": False, "message": f"Error: No permission to delete file: {file_path}", "path": str(path)}
        except Exception as e:
            return {"success": False, "message": f"Error deleting file: {str(e)}", "path": str(path)}
